...
We define Spyre-specific decompositions in [decompositions.py](../torch_spyre/ops/decompositions.py)
...
loop level IR in [lowering.py](../torch_spyre/ops/lowering.py) using the `@register_spyre_lowering` decorator.
...
[customops.py](../torch_spyre/ops/customops.py). This has two pieces:
...
+ register a lowering for the custom op in [lowering.py](../torch_spyre/ops/lowering.py) and
  add a method to `SpyreOpFuncs` in [spyre_kernel.py](../torch_spyre/_inductor/spyre_kernel.py).
  A canonical example is `spyre.clamp`.
+ register a decomposition for the custom op in [decompositions.py](../torch_spyre/ops/decompositions.py)
